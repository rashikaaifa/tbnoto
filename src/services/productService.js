...
async function fetchWithRetry(
	...
	while (true) {
		...
		if (DEBUG)
			console.warn(
				`[429] ${url} → retry in ${delay}ms (attempt ${attempt + 1}/${retries})`
			);
		await sleep(delay);
		attempt++;
	}
...
async function getWithCache(url, { ttl = CACHE_TTL } = {}) {
	...
	if (url === API_URL && productsCache.data && now - productsCache.ts < ttl)
		return productsCache.data;
	if (
		url === CATEGORY_API_URL &&
		categoriesCache.data &&
		now - categoriesCache.ts < ttl
	)
...
const formatCartData = (c) => {
	...
	const rawName =
		...
		b.nama_barang ||
		b.name ||
		`Produk ID ${c.barang_id || b.id || 'unknown'}`;
...
export const addToCart = async (productId, quantity = 1, token) => {
	...
	if (!res.ok) {
		try {
			throw new Error(
				JSON.parse(txt).message || 'Gagal menambahkan ke keranjang'
			);
		} catch {
			throw new Error(
				`Gagal menambahkan ke keranjang (HTTP ${res.status}): ${txt}`
			);
		}
	}
	try {
...
async function flushCartUpdate(cartId, token) {
	...
	try {
		...
		if (!res.ok)
			throw new Error(
				`Gagal mengupdate keranjang (HTTP ${res.status}): ${txt}`
			);
		try {
			JSON.parse(txt);
		} catch {}
		...
		if (DEBUG)
			console.warn('[flushCartUpdate] gagal, reschedule:', e?.message);
...
export function queueUpdateCartItemQuantity(
	cartId,
	quantity,
	token,
	delay = CART_FLUSH_DELAY
) {
	if (!cartUpdateBuffers.has(cartId)) {
		cartUpdateBuffers.set(cartId, {
			latestQty: quantity,
			timer: null,
			inFlight: false,
			lastSentQty: null,
...
export const removeFromCart = async (cartId, token) => {
	...
	if (!res.ok)
		throw new Error(
			`Gagal menghapus dari keranjang (HTTP ${res.status}): ${txt}`
		);
	return true;
};
...
const setProductStockSmart = async (
	productId,
	stokTarget,
	ukuranKnown,
	token
	...
	if (
		...
		!Number.isFinite(payload.harga)
	) {
		throw new Error(
			'Data produk tidak lengkap untuk update stok (nama/kategori/harga kosong).'
		);
	}
	...
	if (res.ok) {
		...
		console.warn(
			'[setProductStockSmart] PATCH full gagal:',
			res.status,
			txt
		);
	}
...
async function flushStockBuffer(productId, token) {
	...
	try {
		...
		if (
			typeof buf.lastStock !== 'number' ||
			typeof buf.lastUkuran !== 'string'
		) {
			const raw = await getProductRaw(productId, token);
		...
		if (DEBUG)
			console.warn('[flushStockBuffer] gagal, requeue', {
				productId,
				err: e?.message,
			});
 ...
 */
export function queueAdjustProductStock(
	productId,
	delta,
	token,
...
export const checkoutCart = async (items, token, extra) => {
	...
	const isTransferMethod = ['transfer_bri', 'transfer_bca'].includes(
		extra?.metode_pembayaran
	);
