...
const ProductPage = () => {
	...
	const sortedProducts = [
		...filteredProducts.filter((product) => product.stok > 0),
		...filteredProducts.filter((product) => product.stok === 0),
	];
	...
	return (
		<div className="mt-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
			<header className={`${isMobile ? 'mb-3' : 'mb-6'}`}>
				<h1
					className={`font-bold ${isMobile ? 'text-xl' : 'text-2xl'} md:text-3xl lg:text-4xl`}
				>
					{selectedCategory
				...
				<p
					className={`${isMobile ? 'text-xs' : 'text-base'} text-gray-600 mt-1`}
				>
					{selectedCategory
			...
			{/* Search & Filter */}
			<div
				className={`${isMobile ? 'flex flex-row space-x-2' : 'flex items-center space-x-4'} mb-6`}
			>
				<div
					className={`${isMobile ? 'flex-1' : 'flex-grow max-w-md'}`}
				>
					<input
						type="text"
