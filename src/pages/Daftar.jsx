...
const Daftar = () => {
  ...
  if (name === "phone_number") {
    // hanya angka tanpa spasi, +62 sudah fixed di depan
    const cleaned = value.replace(/\D/g, ""); // buang non-digit

    // jangan izinkan angka 0 di awal
    if (cleaned.startsWith("0")) {
      return; // abaikan input jika mulai dengan 0
    }

    // batas panjang maksimal
    if (cleaned.length > 12) {
      return; // batasi max 12 digit
    }
  ...
  if (formData.phone_number.length < 9 || formData.phone_number.length > 12) {
    setError("Nomor telepon harus 9–12 digit dan tidak boleh diawali 0.");
    return;
  }
