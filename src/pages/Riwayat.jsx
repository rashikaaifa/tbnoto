...
const RiwayatPage = () => {
				...
				if (Array.isArray(res)) {
					...
					const processedTransactions = res.map((trx) => {
						const processedTrx = {
							...
							harga_satuan:
								trx.harga_satuan ||
								(trx.barang && trx.barang[0]?.harga_satuan) ||
								0,
							quantity:
								trx.quantity ||
								(trx.barang && trx.barang[0]?.quantity) ||
								1,
							subtotal:
								trx.subtotal ||
								(trx.barang && trx.barang[0]?.subtotal) ||
								0,
							tgl_transaksi: trx.tgl_transaksi,
							nama_penerima:
								trx.nama_penerima || 'Tidak diketahui',
							metode_pembayaran:
								trx.metode_pembayaran || 'Tidak diketahui',
	...
	const getProductInfo = (transaction) => {
		if (
			Array.isArray(transaction.barang) &&
		...
		if (
			transaction.barang &&
			typeof transaction.barang === 'object' &&
			!Array.isArray(transaction.barang)
		) {
				...
				<div className="bg-white w-full max-w-2xl rounded-lg shadow-lg relative max-h-[90vh] overflow-y-auto">
					...
					<div className="p-6">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
							<div className="space-y-3">
								<div>
									...
									<p className="text-md">
										{formatTanggal(
											selectedTransaction.tgl_transaksi
										)}
									</p>
								</div>
								<div>
							...
							<div className="space-y-3">
								<div>
									...
									<div>
										<span
											className={`inline-block px-2 py-0.5 rounded-lg text-sm font-medium ${getStatusDisplay(selectedTransaction.status).statusClass}`}
										>
											{
												getStatusDisplay(
						...
						<div className="border-t pt-6">
							...
							selectedTransaction.bukti_transaksi !== 'null' ? (
